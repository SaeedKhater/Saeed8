...
public class Game {
	...
	public void PieceHold(){
		...
		if (holdPiece==null) {
			holdPiece=new Piece(HELD_PIECE_X,HELD_PIECE_Y, nextPiece.getShape());
			this.setNextPiece(PieceGenerator.generatePiece(Game.NEXT_PIECE_X,Game.HELD_PIECE_Y));
		}

		 else {
			Piece tmp = holdPiece;
			holdPiece = new Piece(HELD_PIECE_X,HELD_PIECE_Y, nextPiece.getShape());
			nextPiece= new Piece(nextPiece.getX(), nextPiece.getY(),tmp.getShape());
		}
